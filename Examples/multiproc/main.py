...
class myExperiment(Experiment):
      ...
      # START TRIAL
        ...
        # TRIAL_COMPLETE
            ...
            if self.gTimer[1] > gDeadTime:
                ...
                self.gHardware['gHand'].stopRecording()
    ...
    def onTrialEnd(self):
        ...
        self.gData.add_data_record([gTrial.TN, gDat.run, gTrial.Hand, gTrial.Digit, gVar['Corr'],gVar['TargetX'], gVar['TargetY'],
                                    ...
                                    gVar['measEndTime']])
        mov_data = self.gHardware['gHand'].getBufferCopy()
        self.gData.add_mov_record(mv_data)
        self.gHardware['gHand'].startRecording()

        if gTrial.TN == self.gTrial.getTrialNum:
            self.gHardware['gHand'].terminate()
...
if __name__ == "__main__": 
    ...
    gExp.set_mov_format(['BN', 'TN', 'Dig1X', 'Dig1Y', 'Dig1Z','Dig2X', 'Dig2Y', 'Dig2Z', 'Dig3X',
                        'Dig3Y', 'Dig3Z''Dig4X', 'Dig4Y', 'Dig4Z', 'Dig5X', 'Dig5Y', 'Dig5Z' ])
    ...
    gExp.add_hardware('gHand', HopkinsHandDevice({'device_name': 'hopkins hand device',
                               'sampling_freq': 2,
                               'buffer_size': [10000, 1], 'num_counter': 2}))
